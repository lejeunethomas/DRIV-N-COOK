...
<body data-role="admin">
    <div class="dashboard-layout">
        ...
        <main class="main-content admin">
            ...
            <div class="tabs">
                <button class="tab-btn active" onclick="AdminCommon.utils.switchTab('en_attente', () => loadCommandes('en_attente'))">
                    En attente <span class="badge" id="badge-attente" style="display:none;">0</span>
                </button>
                <button class="tab-btn" onclick="AdminCommon.utils.switchTab('validees', () => loadCommandes('validees'))">
                    Validées <span class="badge" id="badge-validees" style="display:none;">0</span>
                </button>
                <button class="tab-btn" onclick="AdminCommon.utils.switchTab('toutes', () => loadCommandes('toutes'))">Toutes les commandes</button>
            </div>
    ...
    <script>
                ...
                tables: {
                    ...
                    validees: {
                        ...
                        rowBuilder: (commande) => [
                            ...
                            commande.entrepot_nom,
                            `<strong>${AdminCommon.utils.formatPrice(commande.total)}</strong>`,
                            commande.date_livraison_prevue ? AdminCommon.utils.formatDate(commande.date_livraison_prevue) : '-',
        ...
        async function loadCommandes(statut) {
            try {
                const url = statut === 'toutes' ? 
                    commandeManager.config.endpoints.list : 
                    `${commandeManager.config.endpoints.list}?statut=${statut}`;
                    
                const commandes = await AdminCommon.utils.apiRequest(url);
                commandeManager.data[statut] = commandes;
                
                AdminCommon.utils.createTable({
                    containerId: `table-${statut}-container`,
                    headers: commandeManager.config.tables[statut].headers,
                    data: commandes,
                    rowBuilder: (commande) => {
                        const row = document.createElement('tr');
                        const cellsData = commandeManager.config.tables[statut].rowBuilder(commande);
                ...
                if (statut === 'en_attente') updateBadges();
        ...
        function calculateUrgence(commande) {
        ...
        function getStatutBadge(statut) {
            const label = commandeManager.config.statutLabels[statut] || statut;
            return `<span class="statut-badge statut-${statut}">${label}</span>`;
        }

        function getEstimationHTML(commande) {
        ...
        async function voirDetails(commandeId) {
            try {
                const commande = await AdminCommon.utils.apiRequest(`${commandeManager.config.endpoints.details}?id=${commandeId}`);
                ...
                const content = `
                    <div class="detail-section" style="background: #f8f9fa; padding: 1rem; border-radius: 6px; margin-bottom: 1rem;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 0.5rem;">
                            <div><strong>Date de commande:</strong> ${AdminCommon.utils.formatDate(commande.date_commande)}</div>
                            <div><strong>Statut:</strong> ${getStatutBadge(commande.statut)}</div>
                        </div>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 0.5rem;">
                        ...
                        ${commande.date_livraison_prevue ? `<div style="margin-top: 0.5rem;"><strong>Date de livraison prévue:</strong> ${AdminCommon.utils.formatDate(commande.date_livraison_prevue)}</div>` : ''}
                            ...
                            return `
                                <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.8rem; border: 1px solid #ddd; border-radius: 6px; margin-bottom: 0.5rem; background: white;">
                                    ...
                                    <div style="text-align: right;">
                                        ${detail.quantite} × ${AdminCommon.utils.formatPrice(detail.prix_unitaire)}<br>
                                        <strong>${AdminCommon.utils.formatPrice(detail.prix_total)}</strong>
                                    </div>
                                </div>
                            `;
                        }).join('')}
                        <div style="text-align: right; padding-top: 1rem; border-top: 2px solid #1976d2; margin-top: 1rem;">
                            <strong style="font-size: 1.2rem;">Total: ${AdminCommon.utils.formatPrice(commande.total)}</strong>
                        </div>
                    </div>
        ...
        function validerCommande(commandeId) {
            AdminCommon.utils.createFormModal({
                ...
                onSubmit: async (data, isEdit) => {
                    const success = await submitValidation(commandeId, 'valider', data);
                    if (success) {
        ...
        function annulerCommande(commandeId) {
            AdminCommon.utils.createFormModal({
                ...
                onSubmit: async (data, isEdit) => {
                    const success = await submitValidation(commandeId, 'annuler', data);
                    if (success) {
                        AdminCommon.utils.closeModal();
                        await refreshData();
                    }
        ...
        async function marquerLivree(commandeId) {
            ...
            const success = await submitValidation(commandeId, 'livrer');
        ...
        async function submitValidation(commandeId, action, extraData = {}) {
            ...
            try {
                const result = await AdminCommon.utils.apiRequest(commandeManager.config.endpoints.validate, {
                    method: 'POST',
                    data: formData,
                    showLoader: true
                });
        ...
        async function updateBadges() {
            try {
                const [attenteCommandes, valideesCommandes] = await Promise.all([
                    AdminCommon.utils.apiRequest(`${commandeManager.config.endpoints.list}?statut=en_attente`),
                    AdminCommon.utils.apiRequest(`${commandeManager.config.endpoints.list}?statut=validee`)
                ]);
