...
<head>
    ...
    <style>
        .stock-alert-overlay, .stock-warning-absolute, .floating-stock-alert {
...
<body>
    ...
    <script>
        ...
        const CommandeUtils = {
            ...
            getStatutLabel: (statut) => CommandeManager.config.statutLabels[statut] || statut,
            ...
            formatDistance: (km) => km >= 1 ? `${km.toFixed(1)} km` : `${Math.round(km * 1000)} m`,
            ...
            // CRÉATION DE TABLEAUX GÉNÉRIQUE
        ...
        Object.assign(CommandeManager, {
            ...
            displayProduits() {
                ...
                this.data.produits.forEach(produit => {
                    ...
                    const stockBadge = this.getStockBadge(produit);
            ...
            getStockBadge(produit) {
                if (produit.quantite == 0) return this.config.badges.stock.rupture;
                if (produit.quantite <= produit.seuil_alerte) return this.config.badges.stock.faible;
                return this.config.badges.stock.ok;
            },
            ...
            async submit(e) {
                ...
                try {
                    ...
                    if (result.success) {
                        CommandeUtils.showAlert(`Commande créée avec succès ! Total: ${result.total.toFixed(2)}€`, 'success');
                        this.reset();
